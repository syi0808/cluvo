...
export type MockFetch = typeof fetch & ReturnType<typeof mock>

export function createMockFetch(options: MockGitHubOptions = {}): MockFetch {
	const { searchResults = [], createIssueUrl, createIssueError, searchError, latency = 0 } = options
	...
	return mock(async (input: RequestInfo | URL, init?: RequestInit) => {
		...
		if (url.includes('api.github.com/repos/') && init?.method === 'POST') {
			...
			return new Response(JSON.stringify({ html_url: issueUrl, number: 42 }), {
				status: 201,
				headers: { 'Content-Type': 'application/json' },
			})
		}
