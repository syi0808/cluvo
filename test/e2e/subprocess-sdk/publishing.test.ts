...
describe('E2E: subprocess-sdk/publishing', () => {
  ...
  const publishScript = (configOverrides: string, errorExpr: string = `new Error('test error')`) => `
...
`
  ...
  test('4: mode=browser + body > 8000 chars -> file fallback', async () => {
    ...
    const result = await runScript(
      publishScript(`mode: 'browser',`, `new Error('${longMsg}')`),
      { env: environments.pipe, storeDir, prependCode: mockFetchScript() },
    )
    expect(result.exitCode).toBe(0)
    const publishResult = JSON.parse(result.stdout)
