...
import type { MockFetch } from '../helpers/mock-fetch.js'
...
type FetchCall = MockFetch['mock']['calls'][number]
...
describe('E2E: sdk-api/dedup', () => {
	...
	test('1: GitHub search finds similar issues -> matches populated', async () => {
		...
		const promptCtx = presenter.prompt.mock.calls[0]?.[0]
	...
	test('2: user selects react on existing issue', async () => {
		...
		const fetchCalls = (globalThis.fetch as MockFetch).mock.calls
	...
	test('6: dedupe disabled -> search not called', async () => {
		...
		const fetchCalls = (globalThis.fetch as MockFetch).mock.calls
		const searchCalls = fetchCalls.filter((c: FetchCall) => c[0]?.includes?.('search/issues'))
