...
describe('SECTION_RENDERERS', () => {
  ...
  describe('environment', () => {
    ...
    test('includes shell, packageManager, CI when present', () => {
      const result = SECTION_RENDERERS.environment(
        makeReport({ environment: { os: 'linux', arch: 'x64', runtimeVersion: 'v20', shell: '/bin/zsh', packageManager: 'pnpm', ci: true } }),
      )!
  ...
  describe('stackTrace', () => {
    ...
    test('renders stack in code block', () => {
      const result = SECTION_RENDERERS.stackTrace(
        makeReport({ error: { name: 'Error', message: 'fail', stack: 'Error: fail\n  at foo.ts:1:1' } }),
      )!
  ...
  describe('causeChain', () => {
    ...
    test('numbers items', () => {
      const result = SECTION_RENDERERS.causeChain(
        makeReport({ error: { name: 'Error', message: 'top', causeChain: ['mid error', 'root error'] } }),
      )!
