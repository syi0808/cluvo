...
import type { ErrorReport, ReporterConfig } from '../src/types.js'
...
describe('normalizeQuery', () => {
	...
	test('removes Windows-style paths', () => {
		const q = normalizeQuery({
			name: 'Error',
			message: 'Failed at C:\\Users\\me\\project\\file.ts',
		})
...
describe('match', () => {
	test('returns empty result when dedupe is disabled', async () => {
		const result = await match(
			{ error: { name: 'Error', message: 'test' } } as ErrorReport,
			{ repo: 'owner/repo', dedupe: { enabled: false } } as ReporterConfig,
		)
	...
	test('returns empty result on network failure', async () => {
		...
		try {
			const result = await match(
				{ error: { name: 'Error', message: 'test' } } as ErrorReport,
				{ repo: 'owner/repo', dedupe: { enabled: true } } as ReporterConfig,
			)
			expect(result.found).toBe(false)
		} finally {
	...
	test('returns matches from successful GitHub search', async () => {
		...
		try {
			const result = await match(
				{ error: { name: 'TypeError', message: 'x is not a function' } } as ErrorReport,
				{ repo: 'owner/repo', dedupe: { enabled: true } } as ReporterConfig,
			)
